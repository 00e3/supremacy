...
public:
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState* animstate, vec3_t* pos, matrix3x4_t* m) {
		...
		if ((animstate->m_landing || animstate->m_anim_duck_amount != 0.f) && animstate->m_player->GetGroundEntity())
		{
			int bone_id = lookup_bone(animstate->m_player, "head_0");
			if (bone_id == -1)
				return;

			vec3_t bone_pos = m ? vec3_t(m[8][0][3], m[8][1][3], m[8][2][3]) : vec3_t(
				animstate->m_player->m_BoneCache().m_pCachedBones[8][0][3],
				animstate->m_player->m_BoneCache().m_pCachedBones[8][1][3],
				animstate->m_player->m_BoneCache().m_pCachedBones[8][2][3]);
