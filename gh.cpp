...
void DrawBeamPaw(vec3_t src, vec3_t end, Color color)
{
	...
	beamInfo.m_flRed = g_menu.main.visuals.grenade_tracer_warning_color.get().r();
	beamInfo.m_flGreen = g_menu.main.visuals.grenade_tracer_warning_color.get().g();
	beamInfo.m_flBlue = g_menu.main.visuals.grenade_tracer_warning_color.get().b();
...
Color LerpTime(Color a, Color b, float t)
{
	return Color
	(
		a.r() + (b.r() - a.r()) * t,
...
bool c_grenade_prediction::data_t::draw() const
{
	...
	Color negro = g_menu.main.visuals.grenade_tracer_warning_color.get();
	...
	Color outline_warning = g_menu.main.visuals.grenade_tracer_warning_color.get();
	...
	if (dist < 200) {
		...
		vec3_t position_end = std::get< vec3_t >(m_path.at(m_path.size() - 1));
		position_end.z -= 3.f;
		render::world_circle(position_end, 60 + (60 * percent), 1.f, negro);
		render::circle(prev_screen.x, prev_screen.y, 2, 360, Color(LerpTime(Color(242, 65, 65, negro.a()), Color(188, 232, 67, negro.a()), percent)));
	}
