...
public:
	...
	//MultiDropdown head1;
	//Slider        misses;
	Dropdown      velocity_match;
	MultiDropdown extras;
	Slider        baim_hp;
	Keybind       baim_key;
	Keybind       override;
...
public:
	void init() {
		...
		hitbox.setup(XOR("hitbox"), XOR("hitbox"), { XOR("head"), XOR("upper chest"), XOR("chest"), XOR("arms"), XOR("legs"), XOR("feet") });
		RegisterElement(&hitbox);

		multipoint.setup(XOR("aimpoint scaling"), XOR("multipoint"), { XOR("low"), XOR("medium"), XOR("high") }, true);
		...
		seperate_pointscale.setup(XOR("enable individual pointscale"), XOR("seperate_pointscale"));
		RegisterElement(&seperate_pointscale);

		head_scale.setup(XOR("head scale"), XOR("head_scale"), 1.f, 100.f, true, 0, 70.f, 1.f, XOR(L"%"));
		head_scale.AddShowCallback(callbacks::IsSeperateMultipointOn);
		RegisterElement(&head_scale);

		body_scale.setup(XOR("body scale"), XOR("body_scale"), 1.f, 100.f, true, 0, 85.f, 1.f, XOR(L"%"));
		body_scale.AddShowCallback(callbacks::IsSeperateMultipointOn);
		RegisterElement(&body_scale);

		limb_scale.setup(XOR("limb scale"), XOR("limb_scale"), 1.f, 100.f, true, 0, 60.f, 1.f, XOR(L"%"));
		limb_scale.AddShowCallback(callbacks::IsSeperateMultipointOn);
		RegisterElement(&limb_scale);

		minimal_damage.setup(XOR("minimal damage"), XOR("minimal_damage"), 1.f, 100.f, true, 0, 40.f, 1.f, XOR(L" dmg"));
		RegisterElement(&minimal_damage, 1);
		...
		hitchance_zeus_amount.setup(XOR("zeus hitchance"), XOR("hitchance_zeus_amount"), 1.f, 100.f, true, 0, 75.f, 1.f, XOR(L"%"));
		hitchance_zeus_amount.AddShowCallback(callbacks::IsConfigMM);
		RegisterElement(&hitchance_zeus_amount, 1);

		hitchance_amount.setup(XOR("hitchance"), XOR("hitchance_amount"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		...
		//baim2.setup(XOR("strict body aim"), XOR("baim2"), { XOR("always"), XOR("health"), XOR("fake angles"), XOR("high velocity air-bone"), XOR("after x misses") });
		...
		//head1.setup(XOR("non-strict body aim"), XOR("head1"), { XOR("always"), XOR("high velocity anim match"), XOR("enough data"), XOR("high velocity air-bone"), XOR("low velocity air-bone") });
		//RegisterElement(&head1, 1);

		//velocity_match.setup(XOR("velocity match"), XOR("velocity_match"), { XOR("safe"), XOR("medium"), XOR("very high") });
		//velocity_match.AddShowCallback(callbacks::IsVelOn);
		//RegisterElement(&velocity_match, 1);
...
public:
	...
	Checkbox weird_flick;
	MultiDropdown weird_flick_addons;
...
public:
	void init() {
		...
		dir_stand.setup(XOR("direction"), XOR("dir_stnd"), { XOR("auto"), XOR("backwards"), XOR("left"), XOR("right"), XOR("custom"), XOR("fake yaw") });
		...
		weird_flick.setup(XOR("manipulative flick"), XOR("weird_flick"));
		weird_flick.AddShowCallback(callbacks::IsAntiAimModeStand);
		weird_flick.AddShowCallback(callbacks::HasStandYaw);
		RegisterElement(&weird_flick);

		weird_flick_addons.setup(XOR("additions"), XOR("weird_flick_addons"), { XOR("disable on manual") });
		weird_flick_addons.AddShowCallback(callbacks::IsAntiAimModeStand);
		weird_flick_addons.AddShowCallback(callbacks::HasStandYaw);
		RegisterElement(&weird_flick_addons);
		...
		// col2.
		fake_yaw.setup(XOR("fake yaw"), XOR("fake_yaw"), { XOR("off"), XOR("default"), XOR("relative"), XOR("jitter"), XOR("rotate"), XOR("random"), XOR("local view"), XOR("auto dir") });
		...
		lag_active.setup("", XOR("lag_active"), { XOR("move"), XOR("air"), XOR("crouch"), XOR("crouched") }, false);
...
public:
	...
	Checkbox text_weapon;
	Colorpicker text_weaponcol;
	Checkbox weapon_icon;
	Slider weapon_icon_distance;
	Colorpicker weapon_iconcol;
	MultiDropdown flags;
	MultiDropdown flag2s;
	Checkbox ammo_bar;
	Colorpicker ammo_bar_color;
	Checkbox lby_timer;
	Colorpicker lby_timer_color;
	Checkbox distance;
	Colorpicker distance_color;
	Checkbox glow;
	Colorpicker   glow_color;
	Checkbox skeleton;
	Colorpicker skeleton_color;
	Checkbox skeleton_history;
	Colorpicker skeleton_history_col;
	Checkbox out_of_pov;
	Colorpicker out_of_pov_color;
	Slider offscreen_mode;
	Slider offscreen_mode1;

	// col2.
	Dropdown      chams_style;
	Checkbox      chams_enemy_vis_enable;
	Colorpicker   chams_enemy_vis;
	Checkbox      chams_enemy_invis_enable;
	Colorpicker   chams_enemy_invis;
	Colorpicker   chams_enemy_reflectivity;
	Checkbox      chams_enemy_history;
	Dropdown      chams_history_style;
	Colorpicker   chams_enemy_history_col;
	Checkbox chams_friendly_vis_enable;
	Colorpicker   chams_friendly_vis;
	Checkbox chams_friendly_invis_enable;
	Colorpicker   chams_friendly_invis;
	Dropdown      chams_friendly_style;
	Checkbox      chams_local;
	Colorpicker   chams_local_col;
	Dropdown      chams_local_style;
	Checkbox      chams_local_scope;
	Slider        chams_local_scope_amt;
	Checkbox      shot_matrix;
	Dropdown      matrix_shot;
	Colorpicker   matrix_shot_clr_target;
	Colorpicker   matrix_shot_clr;
	Checkbox      matrix_behind_wall;
...
public:
	void init() {
		...
		box_color.setup(XOR("box color"), XOR("box_color"), { 255, 255, 255, 130 });
		box_color.AddShowCallback(callbacks::box_esp);
		RegisterElement(&box_color);
		...
		name_color.setup(XOR("name color"), XOR("name_color"), { 255, 255, 255, 145 });
		name_color.AddShowCallback(callbacks::name);
		RegisterElement(&name_color);

		flags.setup(XOR("flags"), XOR("flags"), { XOR("money"), XOR("armor"), XOR("zoom"), XOR("flashed"), XOR("reload"), XOR("bomb"), XOR("lagcomp"), XOR("ping"), XOR("lethal"), XOR("fake") }, true);
		RegisterElement(&flags);

		text_weapon.setup(XOR("weapon text"), XOR("text_weapon"));
		RegisterElement(&text_weapon);

		text_weaponcol.setup(XOR("weapon text color"), XOR("text_weaponcol"), { 255, 255, 255, 210 });
		text_weaponcol.AddShowCallback(callbacks::weapon_text_on);
		RegisterElement(&text_weaponcol);

		weapon_icon.setup(XOR("weapon icon"), XOR("weapon_icon"));
		RegisterElement(&weapon_icon);

		weapon_iconcol.setup(XOR("weapon icon color"), XOR("weapon_iconcol"), { 255, 255, 255, 210 });
		weapon_iconcol.AddShowCallback(callbacks::weapon_icon_on);
		RegisterElement(&weapon_iconcol);

		ammo_bar.setup(XOR("ammo"), XOR("ammo_bar"));
		RegisterElement(&ammo_bar);

		ammo_bar_color.setup(XOR("ammo bar color"), XOR("ammo_bar_color"), { 80, 140, 200, 190 });
		ammo_bar_color.AddShowCallback(callbacks::ammo_bar);
		RegisterElement(&ammo_bar_color);

		lby_timer.setup(XOR("lby timer"), XOR("lby_timer"));
		RegisterElement(&lby_timer);

		lby_timer_color.setup(XOR("lby timer color"), XOR("lby_timer_color"), { 255, 0, 255 });
		lby_timer_color.AddShowCallback(callbacks::lby_timer);
		RegisterElement(&lby_timer_color);

		glow.setup(XOR("glow"), XOR("glow"));
		RegisterElement(&glow);


		glow_color.setup(XOR("glow color"), XOR("glow_color"), { 180, 57, 118, 153 });
		glow_color.AddShowCallback(callbacks::glow_enabled);
		RegisterElement(&glow_color);

		skeleton.setup(XOR("skeleton"), XOR("skeleton"));
		RegisterElement(&skeleton);

		skeleton_color.setup(XOR("skeleton color"), XOR("skeleton_color"), { 255, 255, 170 });
		skeleton_color.AddShowCallback(callbacks::skeleton);
		RegisterElement(&skeleton_color);

		out_of_pov.setup(XOR("out of pov arrow"), XOR("out_of_pov"));
		RegisterElement(&out_of_pov);

		out_of_pov_color.setup(XOR("out of pov arrow color"), XOR("out_of_pov_color"), colors::white);
		out_of_pov_color.AddShowCallback(callbacks::out_of_pov);
		RegisterElement(&out_of_pov_color);

		offscreen_mode.setup(XOR("offscreen size"), XOR("offscreen_mode"), 20.f, 200.f, true, 0, 100.f, 1.f, XOR(L""));
		offscreen_mode.AddShowCallback(callbacks::out_of_pov);
		...
		offscreen_mode1.AddShowCallback(callbacks::out_of_pov);
		RegisterElement(&offscreen_mode1);

		// col2.

		chams_enemy_vis_enable.setup(XOR("chams"), XOR("chams_enemy_vis_enable"));
		RegisterElement(&chams_enemy_vis_enable, 1);

		chams_enemy_vis.setup(XOR("chams color visible"), XOR("chams_enemy_vis"), { 150, 200, 60 });
		chams_enemy_vis.AddShowCallback(callbacks::chams_on_main);
		RegisterElement(&chams_enemy_vis, 1);

		chams_enemy_invis_enable.setup(XOR("chams through walls"), XOR("chams_enemy_invis_enable"));
		RegisterElement(&chams_enemy_invis_enable, 1);

		chams_enemy_invis.setup(XOR("chams through walls color"), XOR("chams_enemy_invis"), { 60, 120, 180 });
		chams_enemy_invis.AddShowCallback(callbacks::chams_on_main2);
		RegisterElement(&chams_enemy_invis, 1);

		chams_enemy_reflectivity.setup(XOR("chams enemy reflectivity"), XOR("chams_enemy_reflectivity"), { 255, 255, 255 });
		chams_enemy_reflectivity.AddShowCallback(callbacks::chams_on_main);
		RegisterElement(&chams_enemy_reflectivity, 1);

		chams_style.setup(XOR(""), XOR("chams_style"), { XOR("default"), XOR("flat"), XOR("metallic") }, false);
		chams_style.AddShowCallback(callbacks::chams_on_main3);
		RegisterElement(&chams_style, 1);

		skeleton_history.setup(XOR("skeleton history"), XOR("skeleton_history"));
		RegisterElement(&skeleton_history, 1);

		skeleton_history_col.setup(XOR("skeleton history color"), XOR("skeleton_history_col"), { 235, 235, 235 });
		skeleton_history_col.AddShowCallback(callbacks::skeleton_history);
		RegisterElement(&skeleton_history_col, 1);

		chams_local.setup(XOR("local player"), XOR("chams_local"));
		RegisterElement(&chams_local, 1);

		chams_local_col.setup(XOR("local player color"), XOR("chams_local_col"), { 50, 50, 50 });
		chams_local_col.AddShowCallback(callbacks::chams_on_local);
		RegisterElement(&chams_local_col, 1);

		chams_local_style.setup(XOR(""), XOR("chams_local_style"), { XOR("default"), XOR("flat"), XOR("metallic") }, false);
		chams_local_style.AddShowCallback(callbacks::chams_on_local);
		RegisterElement(&chams_local_style, 1);

		chams_local_scope.setup(XOR("local player transparency on scope"), XOR("chams_local_scope"));
		RegisterElement(&chams_local_scope, 1);

		chams_local_scope_amt.setup("", XOR("chams_local_scope_amt"), 0.f, 100.f, false, 0, 50.f, 1.f, XOR(L"%"));
		chams_local_scope_amt.AddShowCallback(callbacks::chams_on_scope);
		RegisterElement(&chams_local_scope_amt, 1);

		chams_enemy_history.setup(XOR("enemy backtrack matrix"), XOR("chams_history"));
		RegisterElement(&chams_enemy_history, 1);

		chams_history_style.setup(XOR(""), XOR("chams_history_style"), { XOR("default"), XOR("flat"), XOR("metallic") }, false);
		chams_history_style.AddShowCallback(callbacks::chams_on_history);
		RegisterElement(&chams_history_style, 1);

		chams_enemy_history_col.setup(XOR("shadows color"), XOR("chams_history_col"), { 255, 255, 200 });
		chams_enemy_history_col.AddShowCallback(callbacks::chams_on_history);
		RegisterElement(&chams_enemy_history_col, 1);


		matrix_shot.setup(XOR("shot hitbox matrix"), XOR("shot_matrix"), { XOR("off"), XOR("all"), XOR("target only"), XOR("target only + all") });
		...
		matrix_behind_wall.setup(XOR("draw matrix behind walls"), XOR("matrix_behind_wall"));
		matrix_behind_wall.AddShowCallback(callbacks::is_shot_matrix_on);
		RegisterElement(&matrix_behind_wall, 1);
	}
};
...
public:
	Checkbox      distance;
	Checkbox      items;
	MultiDropdown items_adds;
	Colorpicker   item_color;
	Colorpicker   item_glow_color;
	Checkbox      proj;
	MultiDropdown proj_adds;
	Colorpicker   proj_color;
	Colorpicker   proj_glow_color;
...
public:
	void init() {
		...
		items_adds.setup("", XOR("proj_adds"), { XOR("text"), XOR("glow") }, false);
		items_adds.AddShowCallback(callbacks::is_items_on);
		RegisterElement(&items_adds);

		item_color.setup(XOR("text color"), XOR("item_color"), colors::white);
		item_color.AddShowCallback(callbacks::is_items_on);
		RegisterElement(&item_color);

		item_glow_color.setup(XOR("glow color"), XOR("item_glow_color"), colors::white);
		item_glow_color.AddShowCallback(callbacks::is_items_on);
		RegisterElement(&item_glow_color);
		...
		proj_adds.setup("", XOR("proj_adds"), { XOR("text"), XOR("glow")}, false);
		proj_adds.AddShowCallback(callbacks::is_proj_on);
		RegisterElement(&proj_adds);

		proj_color.setup(XOR("text color"), XOR("proj_color"), colors::white);
		proj_color.AddShowCallback(callbacks::is_proj_on);
		RegisterElement(&proj_color);

		proj_glow_color.setup(XOR("glow color"), XOR("proj_color"), colors::white);
		proj_glow_color.AddShowCallback(callbacks::is_proj_on);
		RegisterElement(&proj_glow_color);
		...
		grenade_tracer_warning.setup(XOR("projectile proximity warning"), XOR("warning_prox_t"));
		RegisterElement(&grenade_tracer_warning);

		grenade_tracer_warning_color.setup(XOR("color"), XOR("tracer color"), colors::red);
		RegisterElement(&grenade_tracer_warning_color);
		...
		world.setup(XOR("brightness adjustment"), XOR("world"), { XOR("night"), XOR("fullbright"), XOR("ambient light") });
		world.SetCallback(Visuals::ModulateWorld);
		RegisterElement(&world);

		nightcolor.setup("world color", XOR("nightcolor"), { 110, 110, 110 });
		nightcolor.SetCallback(Visuals::ModulateWorld);
		nightcolor.AddShowCallback(callbacks::IsNightMode);
		RegisterElement(&nightcolor);

		propscolor.setup("props color", XOR("propscolor"), { 128, 128, 128 });
		propscolor.SetCallback(Visuals::ModulateWorld);
		RegisterElement(&propscolor);

		ambiencolor.setup("ambien color", XOR("ambiencolor"), { 128, 128, 128 });
		ambiencolor.SetCallback(Visuals::ModulateWorld);
		ambiencolor.AddShowCallback(callbacks::IsAmbien);
		RegisterElement(&ambiencolor);
		...
		// col2.
		removals.setup(XOR("removals"), XOR("removals"), { XOR("visual recoil"), XOR("smoke"), XOR("fog"), XOR("flash"), XOR("scope"), XOR("teammates"), XOR("post proccessing")}, true);
		RegisterElement(&removals,1 );

		enemy_radar.setup(XOR("force enemies on radar"), XOR("enemy_radar"));
		RegisterElement(&enemy_radar, 1);
		...
		client_impact.setup(XOR("client color"), XOR("client_impact"), Color(255, 0, 0, 150));
		client_impact.AddShowCallback(callbacks::is_impact_on);
		RegisterElement(&client_impact, 1);

		server_impact.setup(XOR("server color"), XOR("server_impact"), Color(0, 0, 255, 150));
		server_impact.AddShowCallback(callbacks::is_impact_on);
		...
		grenade_trajectory_color.setup("simulation color", XOR("grenade_trajectory_color"), { 60, 180, 225 });
...
public:
	...
	Checkbox       bunnyhop;
	Checkbox       autostrafe;
	Checkbox       log_purchases;
	Checkbox       log_damage_taken;
	Checkbox       log_damage_given;
	Checkbox       ping_spike;
	Slider		  ping_spike_amt;
	Keybind       sec_ping_spike;
	Slider		  sec_ping_spike_amt;
	Slider ping_spike_interp;
	Keybind       fakewalk;
...
public:
	void init() {
		...
		fov_amt.setup(XOR("fov"), XOR("fov_amt"), 60.f, 140.f, true, 0, 90.f, 1.f, XOR(L"°"));
		RegisterElement(&fov_amt);

		bunnyhop.setup(XOR("auto bunny hop"), XOR("bunnyhop"));
		RegisterElement(&bunnyhop);

		autostrafe.setup(XOR("automatic strafe"), XOR("autostrafe"));
		RegisterElement(&autostrafe);

		log_purchases.setup(XOR("log purchases"), XOR("log_purchases"));
		RegisterElement(&log_purchases);

		log_damage_taken.setup(XOR("log damage taken"), XOR("log_damage_taken"));
		RegisterElement(&log_damage_taken);

		log_damage_given.setup(XOR("log damage given"), XOR("log_damage_given"));
		RegisterElement(&log_damage_given);
		...
		ping_spike_interp.setup(XOR("interpolate loading by"), XOR("ping_spike_interp"), 20.f, 60.f, true, 0, 25.f, 1.f, XOR(L"%"));
		ping_spike_interp.AddShowCallback(callbacks::is_sec_fake_ping_bound);
		ping_spike_interp.AddShowCallback(callbacks::is_fake_ping_bound);
		RegisterElement(&ping_spike_interp);
		...
		autobuy.setup(XOR("auto buy bot"), XOR("autobuy"));
		RegisterElement(&autobuy, 1);
		...
		buy1.AddShowCallback(callbacks::IsAutobuy);
		...
		buy2.AddShowCallback(callbacks::IsAutobuy);
		...
		buy3.AddShowCallback(callbacks::IsAutobuy);
		RegisterElement(&buy3, 1);

		//whitelist.setup(XOR("whitelist cheat users"), XOR("whitelist"));
