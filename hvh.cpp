...
void HVH::GetAntiAimDirection() {
	...
	case 5: //fake yaw
		...
		m_direction = m_auto + 180.f;

		// apply 45 degree jitter.
		m_direction += g_csgo.RandomFloat(-90.f, 90.f);
		break;
...
void HVH::Flick() {
	//static float last_flick_time = 0.0f;
	//static int flick_state = 0;
	//static bool is_stopped = false;


	//bool currently_moving = (g_cl.m_speed > 1.0f || !(g_cl.m_flags & FL_ONGROUND));

	//if (!currently_moving && !is_stopped) {
	//	is_stopped = true;
	//	last_flick_time = g_csgo.m_globals->m_curtime; 
	//	flick_state = 1; 
	//}
	...
	//float time_since_last_flick = g_csgo.m_globals->m_curtime - last_flick_time;


	//if ((flick_state == 1 || flick_state == 2) && time_since_last_flick >= 1.1f) {
	//	*g_cl.m_packet = false;
	//	g_cl.m_cmd->m_view_angles.y = m_auto; 
	//	last_flick_time = g_csgo.m_globals->m_curtime;
	//	flick_state = flick_state == 1 ? 3 : 5; 
	//}


	//if (flick_state >= 3 && time_since_last_flick >= 1.1f) {
	//	if (flick_state % 2 == 0) { 
	//		*g_cl.m_packet = false;
	//		g_cl.m_cmd->m_view_angles.y = m_auto;
	//	}
	...
	//		*g_cl.m_packet = false;
	//
	//		float flick_direction = m_auto > 0 ? 1 : -1;
	//		float high_delta_angle = flick_direction * (70 + (std::rand() % 51 - 25)); 
	//		g_cl.m_cmd->m_view_angles.y += high_delta_angle;
	//	}
	//	last_flick_time = g_csgo.m_globals->m_curtime;
	//	flick_state++;
	//}
	...
	if (g_menu.main.antiaim.weird_flick_addons.get(0))
		if (g_hvh.m_left || g_hvh.m_back || g_hvh.m_front || g_hvh.m_right)
			return;


	if (!currently_moving && !is_stopped) {
		is_stopped = true;
		last_action_time = g_csgo.m_globals->m_curtime;
		action_state = 0;
	}
	...
	float time_since_last_action = g_csgo.m_globals->m_curtime - last_action_time;


	if (is_stopped && (action_state == 0 && time_since_last_action >= 0.2f) ||
		(action_state == 1 && time_since_last_action >= 1.1f)) {

		g_cl.m_cmd->m_view_angles.y = m_auto;
		last_action_time = g_csgo.m_globals->m_curtime;
		action_state = action_state == 0 ? 1 : 2;
	}

	else if (is_stopped && action_state >= 2 && time_since_last_action >= 0.55f) {

		float flick_direction = m_auto > 0 ? -1 : -1;


		m_direction = flick_direction * 140;


		g_cl.m_cmd->m_view_angles.y += m_direction;


		if (time_since_last_action >= 1.1f) {
			last_action_time = g_csgo.m_globals->m_curtime - 0.75f;
			action_state = 1;
		}
		else {
			last_action_time = g_csgo.m_globals->m_curtime;
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air) && !g_input.GetKeyState(g_menu.main.misc.fakewalk.get())) {
	...
	if (g_menu.main.antiaim.weird_flick.get())
		Flick();
