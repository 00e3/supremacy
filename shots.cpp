...
void Shots::OnHurt(IGameEvent *evt) {
	...
	if (g_menu.main.misc.log_damage_given.get()) {
		std::string out = tfm::format(XOR("hit %s in the %s for %i damage (%i)\n"), name, m_groups[group], (int)damage, text.c_str());
		g_notify.add(out);
	}
...
void Shots::OnShotMiss(ShotRecord& shot) {
	...
	if (!target || !target->m_BoneCache().m_pCachedBones)
		return;
	...
	if (!shot.m_record->m_bones || !shot.m_record->m_setup)
		return;
	...
	if (resolver_issue)
	{
		...
		if (shot.m_record->m_mode == Resolver::Modes::RESOLVE_WALK)
			g_notify.add(tfm::format(XOR("missed shot due to animations\n")), colors::gray);
		else if (shot.m_record->m_mode == Resolver::Modes::RESOLVE_AIR && (!trace.m_entity || !trace.m_entity->IsPlayer() || trace.m_entity != target))
			g_notify.add(tfm::format(XOR("missed shot due to fake-lag correction\n")), colors::gray);
		else
			g_notify.add(tfm::format(XOR("missed shot due to fake angles\n")), colors::gray);
	}
		...
		g_notify.add(tfm::format(XOR("missed shot due to unknown\n")), colors::gray);
