...
void Grenades::paint() {
	...
	for (const auto& cur : m_path) {
		...
		if (render::WorldToScreen(prev, screen0) && render::WorldToScreen(cur, screen1))
			render::line(screen0, screen1, g_menu.main.visuals.grenade_trajectory_color.get());
	...
	if (target.second) {
		...
		if (render::WorldToScreen(prev, screen)) {
			if (m_id == FIREBOMB || m_id == MOLOTOV) {
				...
				sprintf(distance_buf, XOR("%.1f"), flFeet);

				if (flFeet <= 7.2f)
					render::esp_small.string(screen.x, screen.y + 5, Color(150, 200, 60, 180), distance_buf, render::ALIGN_CENTER);
				else
					render::esp_small.string(screen.x, screen.y + 5, Color(255, 255, 255, 180), distance_buf, render::ALIGN_CENTER);
			}
		...
		if (m_id == HEGRENADE) {
			if (render::WorldToScreen(prev, screen))
				render::esp_small.string(screen.x, screen.y + 5, dumbo_dagadt, tfm::format(XOR("%i"), (int)target.first) + " dmg", render::ALIGN_CENTER);
		}
	}
...
void Grenades::setup() {
	...
	m_start = math::extrapolate_position(lel, g_cl.m_local->m_vecVelocity(), 15, g_csgo.m_globals->m_interval);
