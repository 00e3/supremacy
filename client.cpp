...
void Client::DrawHUD() {
	...
	//std::string text = tfm::format(XOR("cheat | delay: %ims | tick: %i | %s"), ms, rate, time.str().data());
...
//
//		SetClanTagFn(tag.c_str(), XOR("cheat"));
//	};
//
//	std::string szClanTag = XOR("cheat");
...
//			case 9: {  SetClanTag("cheat"); break; }
//			case 10: { SetClanTag("cheat"); break; }
//			case 11: { SetClanTag("cheat"); break; }
...
void Client::Skybox()
{
	...
	if (g_menu.main.misc.skyboxchange.get()) {
		...
		case 18: //Office
			...
			break;
		case 19: //Office
			sv_skyname->SetValue(XOR("glorious_morning"));
			//game::SetSkybox(XOR("office"));
			break;
		default:
			break;
...
void Client::KillFeed() {
	...
	for (int i{ }; i < size; ++i) {
		...
		if (notice->cheat == 1.5f)
			notice->cheat = FLT_MAX;
	}
}
...
void Client::UpdateInformation() {
	...
	// lol ghetto but makes it look better
	if (state->m_on_ground)
	...
	// TODO: make this have a matrix and make it apply on updatecsa ig?
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::green, XOR("[cheat] "));
